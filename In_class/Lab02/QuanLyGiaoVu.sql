...
ALTER TABLE HocVien ADD CHECK (GETDATE() - YEAR(NgSinh) >= 18);

-- 12. Giảng dạy một môn học ngày bắt đầu (TUNGAY) phải nhỏ hơn ngày kết thúc (DENNGAY).
ALTER TABLE GiangDay ADD CHECK (TuNgay < DenNgay );

-- 13. Giáo viên khi vào làm ít nhất là 22 tuổi.
ALTER TABLE GiaoVien ADD CHECK (GETDATE() - YEAR(NgVL) >= 22);

-- 14. Tất cả các môn học đều có số tín chỉ lý thuyết và tín chỉ thực hành chênh lệch nhau không quá 3.
ALTER TABLE MonHoc ADD CHECK (ABS(TCLT - TCTH) <= 3);

-- 1. In ra danh sách (mã học viên, họ tên, ngày sinh, mã lớp) lớp trưởng của các lớp.
SELECT MaHV, HO, TEN, NgSinh, HOCVIEN.MALOP FROM Lop Join HocVien ON Lop.TRGLOP=HOCVIEN.MAHV;
...
SELECT HOCVIEN.MAHV, Ho, TEn, LANTHI, DIEM FROM KETQUATHI JOIN HOCVIEN ON KETQUATHI.MAHV=HOCVIEN.MAHV WHERE (MAMH='CTRR') ORDER BY Ten, Ho;

-- 3. In ra danh sách những học viên (mã học viên, họ tên) và những môn học mà học viên đó thi lần thứ nhất đã đạt.
SELECT HocVien.MAHV, Ho, Ten, MaMH FROM HocVien JOIN KetQuaThi ON HocVien.MAHV=KETQUATHI.MAHV WHERE (LANTHI=1 AND KQUA='Dat');
...
WHERE (LANTHI=1 AND KQUA='Khong Dat' AND MAMH='CTRR' AND LOP.MALOP='K11');

-- 5. * Danh sách học viên (mã học viên, họ tên) của lớp “K” thi môn CTRR không đạt (ở tất cả các lần thi)
SELECT * FROM HocVien JOIN KetQuaThi ON HocVien.MAHV=KETQUATHI.MAHV
JOIN LOP ON HOCVIEN.MALOP=LOP.MALOP
WHERE (KQUA='Khong Dat' AND MAMH='CTRR' AND LOP.MALOP LIKE 'K%');
